...
The work was divided evenly between us. We contributed via a Github repository. After the work was completed, each of us reviewed the full document before submission to ensure consistency. Ben completed Tasks 1, 4, and 5. Alven completed Tasks 2 and 3.
...
There are no null values, so we can move to the next step. Let's view some summary statistics of the data and visualize its distribution. Afterwards, let's see what we've learned from this information
...
From the summary statistics and the visualization, we can see that the player used that were four different types of modes that were played in this data set. The most commonly played mode was TDM, Team Deathmatch. In total, about 93% of the games were TDM while the remaining 7% were split among the modes, Domination, Hardpoint, and Kill Confirmed. Domination and Kill Confirmed sat at just 1% each. In conclusion, this explains that TDM is a much more favored game mode for this player with Hardpoint sitting in second.
...
cod %>% arrange(
  ...
  mapping = aes(x = fct_infreq(Choice, ordered = TRUE))
) +
  geom_bar( 
    color = "White",
    fill = "red") +
...
From the summary statistics and the visualization, we can see that the Choice of maps played included a wide variety. The top three most common maps played were Crossroads Strike, Diesel, and Rush. Those three maps stood at 7% while the next highest was 6%. All the percentages were pretty close together. From the visualization you can clearly see how many games were played with each of the maps. For example, the top three maps were played 15 times from this data set. The least played map was Amerika, which was played just 3 times.
...
```{r}
model_lm <- lm(Eliminations ~ Deaths, data = cod)
summary(model_lm)
```

From the summary statistics and the visualization, we can see that the relationship between eliminations and deaths is slightly positive. From the linear model, we can conclude that for each additional death, the average change in eliminations is an increase in .06922. From the scatterplot, we can see that there is a weak positive relationship between the two. The points are widely scattered. This weak positive relationship can be summarized by the Multiple R-squared value of .003351. This means that only .003351 of total variation in eliminations can be explained by deaths. This is a very weak relationship. 

While providing individual summary statistics for both variables, we also created a new summary statistic by using the mean value and dividing the two variables. We created a variable called kdr, better known as kill-death ratio. The kdr for each game is calculated by dividing eliminations and deaths. To find the ultimate kdr of this data set, we took the mean value and arrived at approximately 1.11. This means that, on average, the player had 1.11 eliminations for every death during their games. 

## Task 4 - Calculating proportion of game outcomes (win, loss, draw)

```{r}
cod <- cod %>%
  separate(Result, into = c("player_score", "other_score"), sep = "-", convert = TRUE) %>%
  mutate(
    Win = ifelse(player_score > other_score, 1, 0),
    Loss = ifelse(player_score < other_score, 1, 0),
    Draw = ifelse(player_score == other_score, 1, 0)
  ) 

cod %>%
  summarise(
    win_proportion = mean(Win),
    loss_proportion = mean(Loss),
    draw_proportion = mean(Draw)
  )
```

From the above statistics, the player in the dataset won approximately 54% of their games, lost approximately 45%, and drew approximately 1%.

## Task 5 - Answering an inference question with logistic regression

### How does a player's eliminations and deaths explain whether the player won the game?

```{r}
cod <- cod %>%
  select(Win, Eliminations, Deaths) %>%
  na.omit()

model <- glm(Win ~ Eliminations + Deaths, family = binomial, data = cod)
...
$$log(\frac{\hat{p}_i}{1-\hat{p}_i})=.61065 + 0.07082x_{i,eliminations} - 0.10076x_{i,deaths} $$
...
While controlling for the number of deaths, for each additional elimination the player gets, we expect the odds of the player winning to increase by 7.34%, on average.


While controlling for the number of eliminations, for each additional death the player has, we expect the odds of the player winning to decrease by 9.59%, on average.

Let's look at the 95% confidence intervals to make sure that 0 isn't a plausible value for either of the coefficients.

```{r}
confint(model)
```

0 is not in the range of plausible values, determined by the 95% confidence intervals, for either coefficient. This suggests that both effects are statistically significant at the alpha = .05 level.

Overall, these results indicate that a player's performance, in terms of eliminations and deaths, is associated with their likelihood of winning: higher eliminations are associated with higher odds of winning, while higher deaths are associated with lower odds of winning.